      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
            ...
            <tr className="border-b border-slate-200/50 dark:border-slate-700/50">
              {columns.map(col => (
                <th
                  ...
                  className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 ${col.sortable !== false ? 'cursor-pointer hover:text-slate-600 dark:hover:text-slate-300' : ''}`}
                >
          ...
          <tbody>
            ...
            {paged.length === 0 && (
              <tr><td colSpan={columns.length} className="px-4 py-12 text-center text-sm text-slate-500">
            ...
            {Array.from({ length: pages }, (_, i) => (
              <button key={i} onClick={() => setPage(i)}
                className={`flex h-8 w-8 items-center justify-center rounded-lg text-xs font-medium transition-colors ${
                  page === i ? 'bg-indigo-500 text-white' : 'text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800'
                }`}>
                {i + 1}
              </button>
            ))}
