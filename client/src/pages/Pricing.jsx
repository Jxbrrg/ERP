...
const plans = [
  { name: 'Inicial', price: '50.000', period: '/mes', desc: '14 días gratis, luego $50.000/mes - Para empezar', features: ['Todos los módulos', 'Hasta 2 empleados', 'Soporte por email', '100 transacciones/mes'], cta: 'Probar 14 días gratis', popular: false, trial: true },
  { name: 'Negocio', price: '120.000', period: '/mes', desc: 'Precio de Lanzamiento - Para empresas en crecimiento', features: ['Todos los módulos', 'Hasta 15 empleados', 'Soporte prioritario', 'Transacciones ilimitadas', 'API Access', 'Múltiples usuarios', 'Exportación de datos', 'Reportes avanzados'], cta: 'Empezar ahora', popular: true, badge: 'OFERTA LANZAMIENTO' },
  { name: 'Empresarial', price: '230.000', period: '/mes', desc: 'Precio de Lanzamiento - Para grandes organizaciones', features: ['Todo lo de Negocio', 'Empleados ilimitados', 'Soporte 24/7', 'Personalización de marca', 'On-premise opcional', 'SLA garantizado', 'Gerente de cuenta dedicado', 'Seguridad avanzada'], cta: 'Contactar', popular: false, badge: 'OFERTA LANZAMIENTO' },
];
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {plans.map((p, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                ...
                {p.trial && (
                  <p className="mt-1 text-xs text-emerald-400 font-medium">14 días gratis incluido</p>
                )}
                <p className="mt-2 text-sm text-slate-300 font-medium">{p.desc}</p>
