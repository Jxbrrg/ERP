...
const initDb = () => {
  ...
  const migrations = [
    ...
    `ALTER TABLE products ADD COLUMN is_ingredient INTEGER DEFAULT 0`,
    `UPDATE products SET unit = 'unidad' WHERE unit IS NULL`,
    `ALTER TABLE orders ADD COLUMN ingredient_cost REAL DEFAULT 0`,
    `ALTER TABLE orders ADD COLUMN cup_cost REAL DEFAULT 2500`,
    `ALTER TABLE orders ADD COLUMN labor_cost REAL DEFAULT 1000`,
    `ALTER TABLE orders ADD COLUMN supervisor_cost REAL DEFAULT 600`,
    `ALTER TABLE orders ADD COLUMN transport_cost REAL DEFAULT 400`,
