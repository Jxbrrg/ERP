...
console.log('🌱 Sembrando datos demo en NEXUS ERP...\n');
...
nombres.forEach((n, i) => {
  ...
  insertEmp.run(id, code, n, `${n.toLowerCase().replace(' ','.')}@nexus.com`,
...
console.log(`✅ ${nombres.length} empleados creados`);
...
console.log(`✅ Asistencias de 30 días generadas`);
...
cats.forEach(c => {
  const id = uuidv4();
  insertCat.run(id, c, `Categoría de ${c}`);
  catIds.push(id);
});
...
prods.forEach((p, i) => {
  ...
  insertProd.run(id, code, p[0], p[1], pick(catIds), p[2], p[3], p[4] + rand(-10, 20),
...
clientNames.forEach((c, i) => {
  const id = uuidv4();
  insertCust.run(id, `CLI-${String(i + 1).padStart(3, '0')}`, c,
...
for (let i = 0; i < 15; i++) {
  const id = uuidv4();
  insertCust.run(id, `CLI-${String(clientNames.length + i + 1).padStart(3, '0')}`,
...
console.log(`✅ ${custIds.length} clientes creados`);
...
for (let i = 0; i < 50; i++) {
  ...
  insertOrder.run(oid, `ORD-${String(i + 1).padStart(4, '0')}`,
    pick(custIds), pick(employeeIds), total,
...
console.log(`✅ ${ordCount} órdenes creadas con items`);
...
for (let i = 0; i < 80; i++) {
  ...
  insertTx.run(uuidv4(), `TXN-${String(i + 1).padStart(4, '0')}`,
...
console.log(`✅ ${txCount} transacciones contables creadas`);
...
for (let i = 0; i < 40; i++) {
  insertInt.run(uuidv4(), pick(custIds), pick(['call','email','meeting','note','task']),
...
console.log(`✅ 40 interacciones CRM creadas`);
...
projectNames.forEach((n, i) => {
  const id = uuidv4();
  insertProj.run(id, `PROJ-${String(i + 1).padStart(3, '0')}`, n,
    `Proyecto: ${n} - Transformación digital integral`,
...
console.log(`✅ ${projIds.length} proyectos creados`);
...
projIds.forEach(pid => {
  ...
  for (let i = 0; i < numTasks; i++) {
    insertTask.run(uuidv4(), pid, pick(taskNames),
      `Tarea del proyecto asignada al equipo`,
      pick(employeeIds),
...
console.log(`✅ Tareas de proyectos generadas`);
...
const notifs = [
  ...
  ['⚠️ Stock bajo', 'Algunos productos tienen inventario crítico', 'warning'],
...
users.forEach(u => {
  notifs.forEach(n => {
    insertNotif.run(uuidv4(), u.id, n[0], n[1], n[2]);
  });
});
console.log(`✅ Notificaciones de bienvenida creadas`);

console.log('\n🎉 ¡Datos demo sembrados exitosamente!');
